...
run_python_command() {
    if [ "$harness_version" == "v0.4.0" ]; then
        ...
        (time python ./lm-evaluation-harness/main.py --model hf-causal-experimental --model_args $model_args --tasks $1 --device cuda:0 --num_fewshot $2 --batch_size 1 --seed 101) |& tee -a "$output_filename"
    else
